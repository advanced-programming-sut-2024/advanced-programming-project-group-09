...
public class LeaderController {
    ...
    public void unFocus(MouseEvent mouseEvent) {
        ...
        cardName.setText(Gwent.getInstance().getUserLoginIn().getFactionLeader().getName());
        ability.setText(Gwent.getInstance().getUserLoginIn().getFactionLeader().getAbility());
    ...
    public void goToGame(MouseEvent mouseEvent) throws Exception {
        User user = Gwent.getInstance().getUserLoginIn();
        if (user.getCommonCardsInDeck().size() < 22 || user.getSpecialCardsInDeck().size() > 10) {
